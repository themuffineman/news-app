...
const SignUpForm: React.FC = () => {
    ...
    function changeForm(bool:boolean){
        setSignUp(bool)
    }
    ...
    <div className='w-max h-max p-3 border rounded-md shadow-md bg-gray-100 flex flex-col justify-between'>
        <div className='flex flex-col justify-between items-center w-96'>
            <div className='flex justify-between items-center w-full'>
                <div className='p-2 pl-4 w-40 text-3xl capitalize text-left font-semibold cursor-pointer hover:text-gray-500' onClick={()=>changeForm(true)}>sign up</div> 
                <div className='p-2 pr-4 w-40 text-3xl capitalize text-right font-semibold cursor-pointer hover:text-gray-500' onClick={()=>changeForm(false)}>login</div>
            </div>
